...
if(!$_SESSION['id'])
{
   ...
   header("Location: ../index");
    ...
    if($row1['department'] != 'HRM' && $row1['type'] != 'Admin')
	{
	   header("Location: index");
	}
    ...
    if(isset($_POST['department_edit']))
    {
		...
		$access_name = $_POST['dept_name'];  
    
            $stmt = $DB_con->prepare('UPDATE department
                                         SET dept_name=:dept_name,
                                             dept_manager=:dept_manager,
                                             disksize_given=:disksize_given                                             									 
                                       WHERE id=:id');
            $stmt->bindParam(':dept_name',$dept_name); 
            $stmt->bindParam(':dept_manager',$dept_manager);  
            $stmt->bindParam(':disksize_given',$disksize_given);     
            $stmt->bindParam(':id',$id);
                
			$qry = $dbcon->query("SELECT * FROM access where access_name=".$dept_name)->fetch_array();
			$acc = $qry['access_name'];
			
			$sql = "UPDATE access SET access_name = '$acc' WHERE access_name = '$acc'";
				if ($dbcon->query($sql) === TRUE) {
					echo "updaet updated successfully";
				} else {
                     ...
                     <div class="card-body">
                        ...
                        <form method="post" enctype="multipart/form-data" class="form-horizontal">
                              <div class="row">
                                 ...
                                 <div class="form-group col-md-6">
                                    <label for="size">Given Disk:</label>
                                    <select name="givendisk" class="form-control" required>
                                        <option value="<?php echo $disksize_given ?>"><?php echo $disksize_given/1024/1024/1024;echo ' GB' ?></option> 
                                        ...
                                        <option value="322122547200">300 GB</option>
                              ...
                              <button type="submit" class="btn btn-" style="background:#fff200;color:black" name = "department_edit"> Update</button>
                           </form>
                        </div>
                     </div>
                  </div>
