...
if(!$_SESSION['id'])
{
   ...
   header("Location: ../index");
    ...
    //Redirect it to the index if the user is not an Admin
    if($row1['department'] != 'HRM' && $row1['type'] != 'Admin' && $row1['type'] != 'CEO' && $row1['type'] != 'CEO Deputy' && $row1['department'] != 'CEO' && $row1['department'] != 'CEO Deputy')
	{
	   header("Location: index");
	}
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $DB_con->prepare('SELECT dept_name FROM department WHERE id =:id');
		$stmt_select->execute(array(':id'=>$_GET['delete_id']));
	
		$stmt_delete = $DB_con->prepare('DELETE FROM department WHERE id =:id');
		...
		//$sqld = "SELECT access_name FROM access WHERE id:id";
		$qry = $dbcon->query("SELECT * FROM access where id=".$_GET['delete_id'])->fetch_array();
		$acc = $qry['access_name'];
		
		$sql = "UPDATE access SET access_name = '$acc' WHERE id = $user_id";

		$stmt_select1 = $DB_con->prepare('SELECT access_name FROM access WHERE id =:id');
		$stmt_select1->execute(array(':id'=>$_GET['delete_id']));
	
		$stmt_delete1 = $DB_con->prepare('DELETE FROM access WHERE id =:id');
		$stmt_delete1->bindParam(':id',$_GET['delete_id']);
...
<html lang="en">
  <head>
    <meta charset="utf-8">
      ...
      <title>DMS-Department List</title>
         ...
         <div class="row">
            <div class="col-sm-12">
                  <div class="card">
                     ...
                     <div class="card-body">
                           <div class="row justify-content-between">
                              ...
                              <div class="col-sm-6 col-md-6">
                                 ...
                                 <a style="background:#fff200;color:black;" href="department-add" data-toggle="tooltip" data-placement="top" title="" data-original-title="Add new department" href="#"><i class="ri-user-add-line"></i>Add New</a>
                                    <a style="background:black;color:white;" href="disk_usage"> Disk Report </a>
                                 </div>
                              </div>
                           ...
                           <div class="table-responsive">
                              ...
                              $i=1;?>
                                    <tr>
                                       ...
                                       <th scope="col">Given Disk-Size GB</th>
                                       <?php if ($type2 == 'Admin' || $department2 == 'HRM'){?>
                    ...
                    $stmt = $DB_con->prepare('SELECT * FROM department ORDER BY dept_name ASC');
                    $stmt->execute();
                    ...
                    ?>
                                ...
                                <td><?php echo $dept_manager; ?></td>     
                                <td><?php echo ($disksize_given / 1024 / 1024 / 1024).' GB'; ?></td>                                                      
                                <?php if ($type2 == 'Admin' || $department2 == 'HRM'){?>
                                <td>       
                                <div class="flex align-items-center list-user-action">
                                             <!-- <a class="iq-bg-primary" href="department-add.php" data-toggle="tooltip" data-placement="top" title="" data-original-title="Add" href="#"><i class="ri-user-add-line"></i></a> -->
                                             <a class="iq-bg-primary" href="department-edit?edit_id=<?php echo $row['id']; ?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit" href="#"><i class="ri-pencil-line"></i></a>
                                             <a class="iq-bg-warning" href="?delete_id=<?php echo $row['id']; ?>" data-toggle="tooltip" data-placement="top" title="" data-original-title="Delete" onclick="return confirm('Are you sure to remove this Department?')" href="#"><i class="ri-delete-bin-line"></i></a>
                                </div>
