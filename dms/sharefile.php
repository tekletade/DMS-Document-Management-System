...
if(!$_SESSION['id'])
{
   ...
   header("Location: ../index");
	...
	if(isset($_GET['sharef_id']) && !empty($_GET['sharef_id']))
	{
		$id = $_GET['sharef_id'];
		$stmt_edit = $DB_con->prepare('SELECT * FROM files WHERE id =:id');
		$stmt_edit->execute(array(':id'=>$id));
		$share_row = $stmt_edit->fetch(PDO::FETCH_ASSOC);
		extract($share_row);
        $foldername= $share_row['name'];
	}
	else
	{
...
.form-groups input:focus {
  ...
  <body class="  ">
            ...
            <div class="col-xl-3 col-lg-4">
                  <div class="card">
                     <div class="card-header d-flex justify-content-between">
                        <div class="header-title">
                           <h4 class="card-title">Share as you want</h4>
                        </div>
                     </div>
                     <div class="card-body">
                        <form>
                           <div class="form-group text-center">
                              <div class="d-flex justify-content-center">
                                 <div class="crm-profile-img-edit position-relative">
                                    <img class="crm-profile-pic rounded-circssle avatar-100" src="assets/images/favicon.png" alt="profile-pic">
                           ...
                           <div class="img-extension mt-3">
                              <div class="d-inline-block align-items-center">
                                    <span></span>
                                 <a style="color:green"><b>Allowed for Any one Who have an access.</b></a>
                              </div>
                           </div>
                           </div>
                  ...
                  <div class="card">
                     ...
                     <div class="card-body">
                        <div class="new-user-info">
                           <form action="share_done.php" method="post" enctype="multipart/form-data">

                                <input type="hidden" name="user_id" value="<?php echo $share_row['user_id'];?>">
                                <input type="hidden" name="user_name" value="<?php echo $share_row['user_name']; ?>">
                                <input type="hidden" name="name" value="<?php echo $share_row['name'];?>">
                                <input type="hidden" name="department" value="<?php echo $share_row['department'];?>">
                                <input type="hidden" name="parent_id" value="<?php echo $_GET['sharef_id']; ?>">
                                <input type="hidden" name="file_type" value="<?php echo $share_row['file_type']; ?>">
                                <input type="hidden" name="size" value="<?php echo $share_row['size']; ?>">

                                
                                <div class="row">
                                 <div class="form-group col-md-6">
                                    <label for="filename">File Name:<b style= "color:green"> <?php echo $name;?></b></label>
                                    <input type="text" class="form-control" name = "foldername" id="filename" placeholder= <?php echo $department .':Department-file' ?> readonly>
                                 </div>                              
                                 
                                 <div class="form-group col-md-6">
                                    <label for="owner">Owner: <b style= "color:green"><?php echo $user_name;?></b></label>
                                    <input type="text" class="form-control" name = "owner" id="owner" placeholder= <?php echo 'File-type:'.$share_row['file_type'];?> readonly>
                                </div>                               
                                 ...
                                 <ul class="list">
                                    <li>
                                     <div class="header"><i class="ri-share-fill mr-2"></i> &nbsp;Share to Public:</div>
                                        <ul class="content">
                                            <br>
                                            <button type="submit" class="btn btn-" style="background:#fff200;color:black;" name="filetopublic">Done</button>
                                        </ul>
                                   </li>
                                   <br>
                                    
                                   <li>
                                    <div class="header"><i class="ri-share-fill mr-2"></i>&nbsp;Share to Individual:</div>
                                        <ul class="content">
                                        <br>
                                       <select name="sharename" id="select-state" placeholder="Add an Employee...">
                                        <option value="">Add an Employee...</option>
                                        <?php
                                        $query=mysqli_query($dbcon,"select * from users order by id DESC");
                                        ...
                                        <br>
                                        <button type="submit" class="btn btn-" style="background:#fff200;color:black;" name="filetoindividual">Done</button>
                                    ...
                                    <li>
                                        <div class="header"><i class="ri-share-fill mr-2"></i>&nbsp;Share to Department/Group:</div>
                                        <ul class="content">
                                        <br>
                                       <select name="access_name" id="select-state2" placeholder="Add department...">
                                        <option value="">Add department...</option>
                                        <?php
                                        $query=mysqli_query($dbcon,"select * from access order by access_name ASC");
                                            while($row=mysqli_fetch_array($query))
                                            {?>
                                        ...
                                        <br>
                                        <button type="submit" class="btn btn-" style="background:#fff200;color:black;" name="filetogroup">Done</button>
                              ...
                              <!-- <button type="submit" class="btn btn-success" name="share">Done</button> -->
                           </form>
                        </div>
                     </div>
                  </div>
    ...
    <script>
  function toggleNestedFields(optionNumber) {
  var nestedFields = document.getElementById("nestedFields" + optionNumber);
  if (nestedFields.style.display === "none") {
...
Array.from(headers).forEach(function(header) {
  header.addEventListener('click', function() {
    ...
    // Collapse all content lists
    Array.from(headers).forEach(function(otherHeader) {
    ...
    // Toggle the clicked header's open/close state and content visibility
    if (isOpen) {
          ...
          <div class="modal-dialog modal-md">
            <div style="color:white; background-color:#008CBA" class="modal-content">
              <div class="modal-header">
                <h2 style="color:white" class="modal-title" id="myModalLabel">New folder</h2>
              </div>
              ...
              <div class="modal-footer"> 
                ...
                <button style = "color:white;background-color:green"class="btn btn- btn-md" name="folder_save">Create</button>   
   ...
   <!-- Add new file -->
		<div class="modal fade" id="newfile" tabindex="-1" role="dialog" aria-labelledby="myMediulModalLabel">
          <div class="modal-dialog modal-md">
            <div style="color:white;background-color:#008CBA" class="modal-content">
              <div class="modal-header">
                <h2 style="color:white" class="modal-title" id="myModalLabel">Add new file</h2>
              </div>
              <div class="modal-body">
			  <form action="addfile.php" method="post" enctype="multipart/form-data" >
					...
					<label style="color:white" for="subject" class="">Select your file</label>
					...
					<button  type="submit" class="btn btn-success btn-rounded btn-block my-4 waves-effect z-depth-0"  name="file_save" type="submit">UPLOAD</button>
